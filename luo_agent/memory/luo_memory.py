...
class LuoMemory:
    ...
    def __init__(self, palace_path: Optional[str] = None):
        ...
        # register all cells with the network
        for cell in [
    ...
    async def wake_up(self, max_tokens: int = 2000) -> str:
        ...
        Mirrors MemPalace's L0+L1 wake-up strategy:
          - Top semantic facts (permanent knowledge)
        ...
        if recent:
            ...
            for ep in recent[-5:]:
                ts = time.strftime(
                    "%Y-%m-%d %H:%M", time.localtime(ep["timestamp"])
                )
        ...
        # truncate roughly to max_tokens (4 chars ≈ 1 token)
        max_chars = max_tokens * 4
        if len(context) > max_chars:
            context = context[:max_chars] + "\n[...truncated]"

        logger.info(
            f"[luo_memory] wake_up loaded {len(facts)} facts, "
